...
- [ ] スプレッドシートが作成されている
- [ ] 「問合せリスト」という名前のワークシートが存在する
- [ ] 1-2行目にヘッダーが設定されている（問合せNo、タイムスタンプ、問合せ者、概要・詳細、問合せ元Slack URL、タグ1～5、【対象】氏名、【対象】Email、対応期日、オーダ番号、オーダ名、進捗ステータス）
...
- [ ] Bot Token Scopesが設定されている:
  - [ ] `app_mentions:read`
  - [ ] `chat:write`
  - [ ] `users:read`（問合せ者の表示名を取得するために必要）
  - [ ] `team:read`（Slack URLを正しく生成するために必要）
...
3. タイムスタンプ、問合せ者、タグ、対象者情報が正しく記録されているか確認
...
SlackのEvent APIでは、ユーザーが入力したメールアドレスが `<mailto:アドレス|表示>` のように届くことがあります。本システムでは Gemini に渡す前に平文のメール表記へ正規化し、抽出精度を高めています。

### 主な機能

- Slackメンションによる情報入力
- 問合せ者の自動特定（Slack送信者名を記録）
- AIによる自動情報抽出（対象者情報、タグ、期日の正規化）
- 複数依頼の対応（1回のメッセージで複数の依頼を処理）
- Googleスプレッドシートへの自動記録（1件ずつ行を分けて登録、新規行の進捗ステータス列に「1.未着手」を自動設定）
- 処理結果のSlack通知
...
2. ワークシート名を「問合せリスト」に変更（デフォルトの「シート1」から変更）
3. 1-2行目に以下のヘッダーを設定:
   ```
   A1: 問合せNo
   B1: タイムスタンプ
...
- `chat:write` - メッセージ送信
- `users:read` - ユーザー情報の取得（問合せ者の表示名を取得するために必要）
- `team:read` - ワークスペース情報の取得（Slack URLを正しく生成するために必要）

**注意**: 
- `users:read`スコープは、問合せ者をUser IDではなく実際のユーザー名（表示名）で表示するために必要です。このスコープがない場合、問合せ者がUser ID（例：`U05L5RDHLUQ`）で表示されます。
- `team:read`スコープは、スプレッドシートに記録されるSlack URLがブラウザで正しく開けるようにするために必要です。このスコープがない場合、Slack URLが正しく動作しない可能性があります。
...
**文字数制限**: メッセージ本文は1000文字以内で入力してください（コスト削減のため）。

ボットは以下の情報を自動抽出します:
- **対象者情報**: 氏名、メールアドレス
- **タグ**: 問い合わせの属性を表すタグ（最大5つ）。例：「アカウント管理」「アカウント新規申請登録」「スラック」「課題」「作業依頼」など。1つの問い合わせに対して複数のタグを設定できます。
  - **重要**: 問い合わせ内に所属を表す情報（例：「営業のAさん」「SREチームのBさん」など）が明示的に含まれている場合は、その所属情報もタグに含まれます。
    - 例：「営業のAさんのAsanaアカウント追加」→ タグは `["アカウント管理", "新規登録", "Asana", "営業", null]`
- **対応期日**: 作業して欲しい期日が明示されている場合。相対的な表現（例：「１月中」「来月末」「今週末」など）も自動的に正しい日付に変換されます。
- **オーダ情報**: オーダ番号（990000～999999の6桁、または数字2桁+JP+数字6桁形式）とオーダ名がメッセージに含まれている場合、自動的に抽出されます。
- **問合せ者**: 自動的にSlackユーザー名（表示名、実名、またはUser ID）が記録されます

**推奨形式**:
- 「[氏名]さんのアカウント作成をお願い。[メールアドレス]」
- 「[氏名]、[メールアドレス]でアカウント作成」
- 「私のアカウント作成をお願いします。[メールアドレス]」（問合せ者本人のアカウント）
- 「営業の[氏名]さんのAsanaアカウント追加。[メールアドレス]」（所属情報が含まれる場合、自動的にタグに追加されます）
...
```
<@U1234567890> お問合せ頂いた内容について、以下の通りスプレッドシートに1件登録しました。認識相違が無いかご確認ください。
...
```
<@U1234567890> お問合せ頂いた内容について、以下の通りスプレッドシートに2件登録しました。認識相違が無いかご確認ください。
...
**文字数超過**:
```
お問合せの内容が長すぎます（{文字数}文字）。1000文字以内で再度入力してください。
```
...
**原因1: メッセージ形式が不明確**
- 解決策: 対象者のメールアドレスを明確に記載（タグに「アカウント管理」が含まれる場合のみ必須）
- 解決策: 対象者の氏名を明確に記載
...
- 解決策: 1-2行目に正しいヘッダーが設定されているか確認（問合せNo、タイムスタンプ、問合せ者、概要・詳細、問合せ元Slack URL、タグ1～5、【対象】氏名、【対象】Email、対応期日、オーダ番号、オーダ名、進捗ステータス）
...
A. タグに「アカウント管理」が含まれる場合のみ、対象者のメールアドレスが必須です。アカウント作成にはメールアドレスが必要なためです。それ以外の場合は、メールアドレス、氏名は任意です。Slack上では通常どおり `user@example.com` と入力すれば問題ありません（システム側で `<mailto:...>` 形式を平文に直してから AI に渡します）。

### Q1-1. タグとは何ですか？

A. タグは、問い合わせの属性を表す分類です。1つの問い合わせに対して最大5つまでのタグを設定できます。例えば、アカウント管理の問い合わせには「アカウント管理」と「アカウント新規申請登録」のタグを両方設定することができます。タグの例：「アカウント管理」「アカウント新規申請登録」「スラック」「課題」「作業依頼」など。
...
A. 問合せ者は、Slackでメッセージを送信したユーザーです。`users:read` スコープがある場合は `users.info` で表示名・実名などを取得してスプレッドシートの「問合せ者」列に記録します。取得に失敗した場合は User ID（例: `U1234567890`）が記録されます。「私」や「自分」のアカウントという言及がある場合、対象者は問合せ者本人として処理されます。

### Q4. 所属情報はどのように処理されますか？

A. 問い合わせ内に所属を表す情報（例：「営業のAさん」「SREチームのBさん」など）が明示的に含まれている場合、その所属情報は自動的にタグに追加されます。例：「営業のAさんのAsanaアカウント追加」→ タグは `["アカウント管理", "新規登録", "Asana", "営業", null]` となります。
...
A. はい、1000文字以内で入力してください。コスト削減のため、Vertex AIに長すぎる文章を解析させることを防いでいます。1000文字を超える場合はエラーメッセージが返されます。

### Q11. 進捗ステータス（P列）はどう更新されますか？

A. Slackから本システムで新規行が追加されたときだけ、P列に「1.未着手」が自動で入ります。以降のステータス変更はスプレッドシート上で手作業で行ってください（本システムは既存行のP列を上書きしません）。

...
## 11. 課題管理機能の追加・キーワード判定・バリデーション改善・日付型対応（2026年4月）

### 11.1. 課題管理機能の追加（is_issue 分岐）

**ユーザーリクエスト**: 既存の問合せ管理に加えて、課題管理への分岐ロジックを `main.py` 1ファイルのまま追加してほしい。

**要求仕様**:
1. メッセージに課題キーワードが含まれる場合は「課題管理モード」として動作
2. Gemini プロンプトをモードに応じて切り替え
3. スプレッドシートの書き込み先ワークシートと列構成をモードに応じて切り替え
4. Slack 返信メッセージもモードに応じて切り替え

**対応内容**:

- `main.py` の改修:
  - **`extract_info_with_gemini`**: `is_issue: bool = False` 引数を追加。課題管理用プロンプト（`issue_title` / `issue_summary` / `tags` / `priority` / `due_date` を抽出）と問合せ管理用プロンプトを分岐
  - **`write_to_spreadsheet`**: `is_issue: bool = False` 引数を追加。課題管理の場合は「課題管理」シートへ A〜L 列（12列）で書き込み。問合せ管理は「問合せリスト」A〜P 列（16列）を維持
  - **`slack_bot_handler`**: メンション除去・メール正規化直後にキーワード判定で `is_issue` フラグを生成。各関数呼び出しに伝播。返信メッセージも分岐
...
**Slack 返信（課題管理時）**: 「ご報告いただいた課題について、以下の課題管理シートに{N}件登録しました。内容をご確認の上、必要に応じて担当者のアサインをお願いします。」+ 課題タイトル・カテゴリ/タグ・優先度を表示

**ドキュメント更新**:
- `documents/設計書.md`: データフロー、関数シグネチャ、スプレッドシート構造（課題管理シート追加）、Gemini 抽出データ構造を更新
- `documents/要件定義書.md`: システム概要、エントリーポイント仕様、AI 解析ロジック（課題管理プロンプト）、スプレッドシート連携、Slack レスポンスを更新
- `documents/利用マニュアル.md`: チェックリスト（課題管理シート追加）、使用方法（課題管理モードの使い方・例文）、レスポンス例、スプレッドシート確認手順、FAQ（Q12〜Q14 追加）を更新
...
**ユーザーリクエスト（要約）**:
- 問合せの本文中に「【課題】」が含まれる場合の誤検知を防ぎたい → **1行目のみを判定対象**に変更
- 角括弧バリアント（`[課題]` 等）も追加してほしい
- `課題：` `課題事項：` のような表現も認識してほしい
...
- メッセージの1行目のみを対象にキーワードを検索
- 本文の説明文中に課題関連ワードが登場しても誤検知しない

**ドキュメント更新**: 設計書・要件定義書・利用マニュアルのキーワード一覧と「1行目のみ判定」の説明を追記

---

### 11.3. メールアドレスバリデーションのスキップ機能

**ユーザーリクエスト**: アカウント管理に関連する問合せでもメールアドレスが不要なケースがある。1行目に「メールアドレス無し」「アカウント管理ではない」等の文言があれば、バリデーションをスキップして登録できるようにしたい。
...
if not is_issue and not skip_email_validation:
    # アカウント管理タグ + メールアドレスなし の場合のみエラー
```

- キーワードも1行目のみを判定対象とする
- 課題管理モード（`is_issue=True`）は引き続きバリデーションをスキップ

**ドキュメント更新**: 利用マニュアルのエラー例に「バリデーションをスキップする方法」のヒントを追記。設計書・要件定義書のバリデーション説明を更新

---

### 11.4. 課題管理シートの日付型対応

**ユーザーリクエスト**: 課題管理シートの起票日時と目標解決日が文字列で入っているので、日時型として入力されるようにしたい。

**対応内容**:

- 2つのヘルパー関数を追加:
  - `_datetime_to_sheets_serial(dt)`: `datetime` → Google Sheets シリアル値（浮動小数）
  - `_date_str_to_sheets_serial(date_str)`: `"YYYY-MM-DD"` → Google Sheets シリアル値（整数）
- Google Sheets のエポックは 1899-12-30。シリアル値を `RAW` で書き込むとスプレッドシートが日付型として認識する
- B列（起票日時）: `_datetime_to_sheets_serial(datetime.now(jst))` に変更
- J列（目標解決日）: `YYYY-MM-DD` 形式の場合はシリアル値、それ以外は文字列フォールバック

**ドキュメント更新**: 設計書 `write_to_spreadsheet` 節に「日付型の扱い」の説明を追記

---

**このセッションで更新されたファイル**:
- `main.py`: 課題管理分岐（`is_issue`）、キーワード判定改善、メールアドレスバリデーションスキップ、日付シリアル値変換の実装
- `documents/設計書.md`: 全面更新（データフロー、関数設計、スプレッドシート構造、Gemini 抽出データ構造、日付型対応）
- `documents/要件定義書.md`: 課題管理分岐、キーワード判定、バリデーション改善を反映
- `documents/利用マニュアル.md`: チェックリスト、使用方法、レスポンス例、FAQ を更新
- `documents/Cursorやり取り纏め.md`: 本セッションの記録を追加
...
| 変更点 | 内容 |
|:--|:--|
| 課題管理モード | `is_issue` フラグで Gemini プロンプト・スプレッドシート書き込み・Slack 返信を分岐 |
| キーワード判定 | 1行目のみ対象、角括弧バリアント・`課題：`/`課題事項：` を追加 |
| バリデーションスキップ | 1行目に「メールアドレス無し」等のキーワードがあればメールアドレスチェックをスキップ |
| 日付型書き込み | 課題管理シートの起票日時・目標解決日を Google Sheets シリアル値で書き込み |

---

## 10. Slack mailto正規化・プロンプト強化・進捗ステータス列（2026年3月）

**背景**: タグに「アカウント管理」が付く依頼で、本文にメールを書いているのに `target_email` が空になりバリデーションエラーになる事象が多発。Slackの `<mailto:...>` 表記が原因の可能性が高い。

**ユーザーリクエスト（要約）**:
1. Gemini に渡す前に Slack 特有の mailto リンクを平文に戻す。
2. モデルを `gemini-3.1-flash-lite-preview` に変更する案は試したがリージョン等で利用不可のため、`gemini-2.5-flash-lite` に維持。
3. プロンプトの【対象者情報】でメール抽出条件を強化。
4. 新規問い合わせ行の P 列（進捗ステータス）に初動として `1.未着手` を書き込む。

**`main.py` の主な変更**:
- `normalize_slack_mailto_links()` を追加。`app_mention` 処理でメンション除去直後に適用。
- `write_to_spreadsheet`: 行データ末尾に `1.未着手` を追加。更新範囲を `A:P` に変更。
- Slack 成功メッセージのスプレッドシート範囲リンクを `A:P` に合わせる。

**ドキュメント修正（列順の実態に合わせた整理を含む）**:
- スプレッドシート列は **D=概要・詳細、E=問合せ元Slack URL、F～J=タグ1～5、K～O=対象者・期日・オーダ、P=進捗ステータス** とコード実装を一致させて記載（従来ドキュメントの D/E と M の対応が誤っていたため修正）。
- `documents/設計書.md`: データフロー、`slack_bot_handler` フロー、`normalize_slack_mailto_links`、`write_to_spreadsheet`、スプレッドシート構造表を更新。
- `documents/要件定義書.md`: Slack mailto 正規化、カラム構成（A～P）、成功時リンク範囲の記述を更新。
- `documents/利用マニュアル.md`: ヘッダー例、チェックリスト、概要・確認項目、トラブルシューティングのヘッダー列挙、Q1/Q3/Q11 を更新。
- `documents/トラブルシューティング履歴.md`: 上記 mailto 問題の項目を追加。
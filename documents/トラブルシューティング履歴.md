...
6. [アカウント管理でメールを書いたのに必須エラーになる（Slackのmailto表記）](#6-アカウント管理でメールを書いたのに必須エラーになるslackのmailto表記)
...
## 6. アカウント管理でメールを書いたのに必須エラーになる（Slackのmailto表記）

### 発生時期・背景
SlackのEvent APIでは、ユーザーが `user@example.com` と入力したメッセージ本文が、`<mailto:user@example.com|user@example.com>` のように届くことがある。

### 症状
- 本文に氏名・メールの両方を書いているのに、ボットから「アカウント管理の依頼には対象者のメールアドレスが必要です。メールアドレスを含めて再度入力してください」と返る。
- Gemini が `target_email` を `null` にし、バリデーションで弾かれる。

### 原因
- 上記の `<mailto:...>` 表記のまま AI に渡すと、軽量モデルがメールとして正しく抽出できず `null` になることがあった。

### 解決方法
- `main.py` の `normalize_slack_mailto_links()` を、メンション除去の直後・`extract_info_with_gemini` 呼び出し前に実行する（本リポジトリの現行実装）。
- 併せて、プロンプトの【対象者情報】でメール抽出条件を明確化している。
- デプロイが古い場合は最新の `main.py` を再デプロイする。

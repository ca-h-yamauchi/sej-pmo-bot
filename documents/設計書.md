...
**戻り値**: `[{"target_name": str|null, "target_email": str|null, "target_team": str|null, "tags": List[str]|null, "details": str|null, "due_date": str|null}, ...]`
...
- 各行の構成: `[問合せNo, タイムスタンプ, 問合せ者, 問合せ元Slack URL, タグ1, タグ2, タグ3, タグ4, タグ5, 【対象】氏名, 【対象】Email, 【対象】所属チーム, 対応期日, 概要・詳細, 元のメッセージ]`
...
| L | **【対象】所属チーム** | `target_team`（正規化済み、空欄可） |
| M | 対応期日 | `due_date`（YYYY-MM-DD形式、空欄可） |
| N | 概要・詳細 | `details`（空欄可） |
| O | 元のメッセージ | Slackから受信した元のテキスト |
